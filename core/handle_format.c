...
/*   Updated: 2018/08/18 11:48:04 by besteba          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	handle_format(t_format *form, va_list ap)
{
	...
	str = tmp;
	if (!form->precision && form->flag == '0'
		&& is_in_str("xXuUoOidD", form->specifier) && form->width)
		add_zero(&tmp, str, form);
	if ((is_in_str("#+", form->flag) || form->is_negative)
		&& is_in_str("xXuUoOidD", form->specifier))
